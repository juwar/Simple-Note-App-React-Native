...
class Add extends Component {
    ...
    createNotes = (title, notes, category) => {
        if (title != '', notes != '', category != '') {
            this.props.dispatch(postNotes(title, notes, category))
            this.props.dispatch(getNotes())
            console.warn(`Success Success Success POST ${title} ${notes} id = ${category}`)
        } else {
        ...
        return (
            <Container>
                <Header style={{ backgroundColor: 'white' }}>
                    ...
                    <Body style={{ flex: 9 }}>
                        <Title style={{ alignSelf: 'center', color: 'black', }}>Add Note</Title>
                    </Body>
                    <Right style={{ flex: 1 }}>
                        <TouchableOpacity onPress={() => (this.createNotes(this.state.title, this.state.note, this.state.selected), this.props.navigation.navigate('Home'))}>
